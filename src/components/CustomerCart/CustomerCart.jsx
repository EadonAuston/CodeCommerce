   ...
   setQuantity = ({target}) => {
     this.props.setQuantity(target);
      this.setState({
         ...
         [`totalPrice${target.id}`]: target.selectedOptions[0].label * this.state[`basePrice${target.id}`],
         
         
      })
      return target.selectedOptions[0].label
   }
            ...
            <div className={flexbox}>
               <div className={style.product}>
                  ...
                  {this.props.cartData ? this.props.cartData.map((item) => (
                        ...
                        size={item.size}
                        price={item.price}
                        totalPrice={(this.props[`totalPrice${item.id}`]).toFixed(2)}
